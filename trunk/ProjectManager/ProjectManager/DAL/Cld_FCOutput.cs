...
namespace TDK.Core.Logic.DAL
{
	...
	/// </summary>
	[Serializable]
	public partial class Cld_FCOutput　: EntityBase
	{
		...
		public virtual Cld_FCBlock Cld_FCBlock
		{
			get
            {
                ...
                {
                    if (Cld_FCOutput.session != null)
                    {
						ITransaction transaction = Cld_FCOutput.session.BeginTransaction();
						try{
							this.m_Cld_FCBlock.Value = 
								Cld_FCOutput.session.Get<Cld_FCBlock>(this.Cld_FCBlock_ID);
							transaction.Commit();
                    ...
                    if (Cld_FCOutput.session != null)
                    {
						...
						try{
							this.m_Prj_Sheet.Value = 
								Cld_FCOutput.session.Get<Prj_Sheet>(this.Prj_Sheet_ID);
							transaction.Commit();
                    ...
                    if (Cld_FCOutput.session != null)
                    {
						...
						try{
							this.m_Prj_Document.Value = 
								Cld_FCOutput.session.Get<Prj_Document>(this.Prj_Document_ID);
							transaction.Commit();
                    ...
                    if (Cld_FCOutput.session != null)
                    {
						...
						try{
							this.m_Prj_Controller.Value = 
								Cld_FCOutput.session.Get<Prj_Controller>(this.Prj_Controller_ID);
							transaction.Commit();
